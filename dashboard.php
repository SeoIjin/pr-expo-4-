...
$today = date('Y-m-d');
$sql_today = "SELECT COUNT(*) AS today_bookings FROM bookings";
...
$start_of_week = date('Y-m-d', strtotime('last Sunday')); // Get the previous Sunday
$end_of_week = date('Y-m-d', strtotime('next Saturday')); // Get the next Saturday
$sql_week = "SELECT COUNT(*) AS weekly_bookings FROM bookings";
...
$sql_month = "SELECT COUNT(*) AS monthly_bookings FROM bookings";
...
<body>
    ...
    const bookingsData = [
      { bookings: 0 },  // Monday
      { bookings: <?php echo $today_bookings; ?> },  // Tuesday
